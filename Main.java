...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 * Simple iteration in the Final array to remove the unnecessary Points that are on the 
			 * same line with on other one and are conquered by the first. 
			 */
			for(int count=Final.size() -1; count >=1; count--){
				
				if(Final.get(count).getX()==Final.get(count-1).getX()){
					//System.out.println("del : "+Final.get(count-1).getX()+"  "+Final.get(count-1).getY());
					Final.remove(Final.get(count-1));
				}
				if(Final.get(count).getY()==Final.get(count-1).getY()){
					Final.remove(Final.get(count-1));
				}
